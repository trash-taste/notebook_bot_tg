...
packages/storage/       # SQLAlchemy models, repositories, JSONL import
...
SQLite — источник истины для новых данных. Новая storage-схема создаётся через SQLAlchemy:

- `capture_items` — сырой вход пользователя до LLM;
- `notes` — разобранные заметки/items;
- `tasks` — задачи;
- `reminders` — будущие напоминания.

Главное правило: бот сначала создаёт `capture_items`, потом вызывает OpenRouter. Если LLM упадёт, raw text остаётся в SQLite со статусом `parse_failed`.
...
Импорт старых JSONL не удаляет исходники:

```bash
python scripts/import_jsonl_to_sqlite.py --db data/notes.db --notes data/notes.jsonl --tasks data/tasks.jsonl
```

...
from src.voice_note_parser.config import Settings
...
class ServiceTests(unittest.TestCase):
    def test_parse_transcript_calls_prompt_openrouter_and_validation(self) -> None:
        ...
        with (
            patch("src.voice_note_parser.service.render_prompt", return_value="prompt") as render,
            patch(
                "src.voice_note_parser.service.request_completion",
                return_value='{"raw_text":"ok"}',
            ) as request,
            patch(
                "src.voice_note_parser.service.parse_model_json",
                return_value={"raw_text": "ok"},
            ) as parse_json,
            patch(
                "src.voice_note_parser.service.validate_parser_response",
                return_value=payload,
            ) as validate,
        ):
            result = parse_transcript("  завтра купить магний  ", settings=test_settings())
    ...
    def test_parse_transcript_rejects_empty_text(self) -> None:
        with self.assertRaises(ParserInputError):
            parse_transcript("  ", settings=test_settings())
